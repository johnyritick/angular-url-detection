...
export class BulkUrlDetectorComponent implements OnInit {
  ...
  submitFile() {
    ...
    if(this.file && this.file.name) {
      ...
      this.http.post('http://0.0.0.0:80/ml/predict-csv', formData).subscribe({
        next: (response: any) => {
          this.openSnackBar(response.message ?? "Uploaded Successfully, we will send you email once file is ready" , 'Ok', true)
          this.clearFile()
        },
