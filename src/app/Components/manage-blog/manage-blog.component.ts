...
export class ManageBlogComponent implements OnInit {
  ...
  getBlogData() {
    this.http.get("http://0.0.0.0:80/admin/get-all-blogs").subscribe({
      next: (response: any) => {
        if (response.success) {
          let temp: {}[] = []
  ...
  deleteBlogData(blog_id: string) {
    this.http.delete("http://0.0.0.0:80/admin/delete-blog/" + blog_id).subscribe({
      next: (response: any) => {
        if (response.success) {
  ...
  createPost() {
    this.http.post("http://0.0.0.0:80/admin/create-blog", {
      "url": this.url,
