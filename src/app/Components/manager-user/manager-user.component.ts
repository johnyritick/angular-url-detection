...
export class ManagerUserComponent implements OnInit {
  ...
  displayedColumns: string[] = [
    "s_no", "name", "email", "user_id", "action"
  ];
  ...
  registerUser() {
    ...
    if (validate.success) {
      this.userService.register({ full_name: this.formData.name, email: this.formData.email, password: this.formData.password }).subscribe({
        next: (response: any) => {
          if (response.success) {
        ...
        error: (error) => {
          ...
          this.openSnackBar(error?.error?.message ?? "Failed to create user", 'Ok')
        }
      })
    } else {
  ...
  deleteUser(user_details: any) {
    this.loading = true
    this.http.delete("http://0.0.0.0:80/admin/user/" + user_details.user_id).subscribe({
      next: (response: any) => {
        if (response.success) {
          this.openSnackBar(response.message, "Ok", true)
          this.getUsersData()
  ...
  updateUser() {
    ...
    if(validate.success) {
      this.loading = true
      this.http.put("http://0.0.0.0:80/admin/user/" + this.formData.user_id,
        {
          full_name: this.formData.name, email: this.formData.email, password: this.formData.password
        }).subscribe({
          next: (response: any) => {
            if (response.success) {
              this.openSnackBar(response.message, "Ok", true)
